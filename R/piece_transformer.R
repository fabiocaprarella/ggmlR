...
pt_transformer <- function(layout, d_model = 256L, n_heads = 8L, n_layers = 4L,
  ...
  blocks <- lapply(seq_len(n_layers), function(i) {
    list(norm1 = list(g = rep(1, d_model), b = numeric(d_model)),
         ...
         attn  = list(w_q = matrix(0.0, d_model, d_model),
                      ...
                      b_o = numeric(d_model)),
         ff1   = list(w = matrix(0.0, ff_dim, d_model), b = numeric(ff_dim)),
         ff2   = list(w = matrix(0.0, d_model, ff_dim), b = numeric(d_model)))
  })
  ...
  structure(list(
    ...
    proj_w      = matrix(0.0, d_model, max_slot * d_model),
    ...
    out_w       = matrix(0.0, n_actions, d_model),
...
# A linear layer's weight, from the checkpoint to ag_linear's [out, in].
# pth_load() hands back the reverse of torch's shape, so this puts it back.
.pt_w <- function(w) t(w)
...
pt_transformer_load <- function(path, layout = pt_cube4_layout(), ...) {
  ...
  for (i in seq_len(model$n_layers)) {
    ...
    # Q, K and V share one tensor in torch, stacked in that order.
    inw <- .pt_w(need(paste0(p, "attn.in_proj_weight")))   # [3d, d]
    ...
    model$blocks[[i]]$attn <- list(
      ...
      w_o = .pt_w(need(paste0(p, "attn.out_proj.weight"))),
      b_o = as.vector(need(paste0(p, "attn.out_proj.bias")))
    )

    model$blocks[[i]]$ff1 <- list(w = .pt_w(need(paste0(p, "ff.0.weight"))),
                                  b = as.vector(need(paste0(p, "ff.0.bias"))))
    model$blocks[[i]]$ff2 <- list(w = .pt_w(need(paste0(p, "ff.3.weight"))),
...
.pt_layer_norm <- function(x, g, b, eps = 1e-5) {
  # x is [tokens, d_model]; normalise each token over d_model.
  mu <- rowMeans(x)
  xc <- x - mu
  sd <- sqrt(rowMeans(xc * xc) + eps)
  sweep(sweep(xc / sd, 2L, g, "*"), 2L, b, "+")
}
...
.pt_attention <- function(h, a, n_heads) {
  # h is [tokens, d_model]; torch's batch_first layout, kept throughout.
  d <- ncol(h)
  dk <- d %/% n_heads

  q <- sweep(h %*% t(a$w_q), 2L, a$b_q, "+")
  k <- sweep(h %*% t(a$w_k), 2L, a$b_k, "+")
  v <- sweep(h %*% t(a$w_v), 2L, a$b_v, "+")

  out <- matrix(0.0, nrow(h), d)
  for (i in seq_len(n_heads)) {
    cols <- seq.int((i - 1L) * dk + 1L, i * dk)
    scores <- (q[, cols, drop = FALSE] %*% t(k[, cols, drop = FALSE])) / sqrt(dk)
    out[, cols] <- .pt_softmax_rows(scores) %*% v[, cols, drop = FALSE]
  }
  sweep(out %*% t(a$w_o), 2L, a$b_o, "+")
}

#' Run a piece transformer
#'
#' Reads one state and returns a value per action.
#'
#' @param model A \code{pt_transformer}, normally from
#'   \code{\link{pt_transformer_load}}.
#' @param state Integer vector of \code{state_size} colours, each in
#'   \code{0..num_classes-1}. A solved 4x4x4 is \code{rep(0:5, each = 16)}.
#' @return A numeric vector of \code{n_actions} values, one per move.
#' @export
...
pt_forward <- function(model, state) {
  ...
  if (length(state) != lay$state_size)
    stop("pt: state has ", length(state), " entries, expected ", lay$state_size,
         call. = FALSE)
  if (any(state < 0L | state >= lay$num_classes))
    stop("pt: colours must be 0..", lay$num_classes - 1L, call. = FALSE)

  n_pieces <- model$n_pieces
  max_slot <- model$max_slot

  # Each piece's stickers, as colours. [n_pieces, max_slot]
  ...
  # Gather, zero the padding slots, and lay the slots of a piece side by side.
  emb <- array(t(model$value_embed[, as.vector(idx) + 1L, drop = FALSE]),
               dim = c(n_pieces, max_slot, model$d_model))
  emb <- emb * array(lay$mask, dim = dim(emb))
  flat <- matrix(aperm(emb, c(1L, 3L, 2L)), n_pieces, max_slot * model$d_model)

  h <- sweep(flat %*% t(model$proj_w), 2L, model$proj_b, "+")
  h <- h + t(model$pos_embed)
  h <- h + t(model$type_embed[, lay$types + 1L, drop = FALSE])

  # The CLS token leads, and its row is what the output is read from.
  h <- rbind(model$cls_token, h)
  h <- .pt_layer_norm(h, model$input_norm$g, model$input_norm$b)

  for (blk in model$blocks) {
    h <- h + .pt_attention(.pt_layer_norm(h, blk$norm1$g, blk$norm1$b),
                           blk$attn, model$n_heads)
    x <- .pt_layer_norm(h, blk$norm2$g, blk$norm2$b)
    x <- .pt_act(sweep(x %*% t(blk$ff1$w), 2L, blk$ff1$b, "+"), model$activation)
    h <- h + sweep(x %*% t(blk$ff2$w), 2L, blk$ff2$b, "+")
  }

  pooled <- .pt_layer_norm(h, model$output_norm$g, model$output_norm$b)[1L, ]
  as.vector(model$out_w %*% pooled) + model$out_b
}
